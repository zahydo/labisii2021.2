...
public class MotoParkingCost implements IParkingCost{
    ...
    /**
     * calcula el costo del parking para Moto
    ...
    public long calculateCost(Vehicle veh, LocalDateTime input, LocalDateTime output) {
        ...
        if(horas < 1){
            costo = costoHora - 1000;
        }else if(horas == 1){
            costo =((minutos/60)* (costoHora-1000)) + costoHora ;
            
        }else{
            costo = costoHora + ((horas-1)*(costoHora-1000)) + ((minutos/60)*(costoHora-1000));
        }
