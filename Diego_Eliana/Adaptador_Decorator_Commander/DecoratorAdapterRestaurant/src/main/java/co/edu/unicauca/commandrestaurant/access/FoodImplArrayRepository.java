...
public class FoodImplArrayRepository implements IFoodRepository {
    ...
    private void initData() {
        components.add(new CryptFood(new CapitalFood(new BaseFood(1, "Fríjoles", FoodTypeEnum.PRINCIPIO))));
        components.add(new CryptFood(new CapitalFood(new BaseFood(2, "Sopa de verduras", FoodTypeEnum.ENTRADA))));
        components.add(new CryptFood(new CapitalFood(new BaseFood(3, "Jugo de mango", FoodTypeEnum.JUGO))));
    ...
    @Override
    public boolean undoDelete(Food component) {
        if (findById(component.getId()) != null)
            return false;
